...
- Suggesting a tool (use **Suggest a Tool** below).
- Suggesting a prompt template or skill (use **Suggest a Template** below).
...
The template’s frontmatter sets the default title prefix and labels (see the file for the current value).

### Template and skill suggestions

For **new prompt templates or skills** (or similar reusable developer-facing content), open an issue and choose **“Suggest a Template”**. That template (see [.github/ISSUE_TEMPLATE/suggest_template.md](.github/ISSUE_TEMPLATE/suggest_template.md)) expects:

| Section | What to include |
|--------|------------------|
| **Title** | Prefix with `[TEMPLATE]` (per the template default). |
| **Template name** | Clear name for the template or skill. |
| **Category** | Same category options as tool suggestions (Text Generation, Code Generation, Testing/Debugging, Documentation, Code Review, or Other). |
| **Why recommend it** | 2–3 sentences on the developer problem it solves. |
| **Usage** | One or two concrete examples of when someone would use it. |
| **Comparison** | How it differs from or complements templates already in this hub. |
| **Content** | Either a link to the template or the full text pasted in the issue (so reviewers can assess fit and quality). |

The template applies the label **`template-suggestion`** when used via GitHub’s issue chooser.

### Other changes

For corrections, new sections, or work that is neither a tool nor a template suggestion, open a **blank issue**. Describe the change, link to affected files, and note whether you are willing to open a PR.
...
- [ ] Template or skill idea? Use **Suggest a Template** and include **Content** (link or paste) plus **Usage** examples.
